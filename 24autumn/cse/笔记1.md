...
14. 什么是一个 file handler？是指明要访问服务器上的哪个文件
    ...
    - 所以最终设计出来的 fh（file handler） 就包括了 inode number 和 generation number
...
5. 第一个 Multi site 场景：high-layer tx 和 low-layer tx

6. 第二个 Multi site 场景：跨机器数据的事务
...
10. 修改：需要把 low layer 的 commit log entry 变成 tentatively commit log entry（PrePared）

    - 还会包含一个 reference 到 higher tx 的 ip，如果可以提交、就变成 commit 状态

11. 分布式下会有一个 partial 的问题
...
12. 流程：为了让这个所有 low level tx 要么都提交、要么都不提交
...
13. 出错处理：
    ...
    - worker failure：如果 restart、会重新问 coordinator，然后根据 coordinator 的 log 来决定
...
14. 为什么 prepare 完之后就能在 high level 里面记录 commit log 了？
...
17. In two-phase commit, failures prior to the commit point can be aborted. If workers (or the coordinator) fail after the commit point, they recover into the PREPARED state, and complete the transaction

18. 问题在于 coordinator 挂了，就不可用了，所以需要 replication 保证高可用
...
20. 分为 optimistic replication 和 pessimistic replication

21. Optimistic Replication (e.g., eventual consistency)

    - Tolerate inconsistency, and fix things up later
    - Works well when out-of-sync replicas are acceptable

22. Pessimistic Replication (e.g., linearizability)

    - Ensure strong consistency between replicas
    - Needed when out-of-sync replicas can cause serious problems

23. Some applications may prefer not to tolerate inconsistency
...
24. Trade-off: stronger consistency with pessimistic replication means:
...
25. RSM (Replicated State Machine) 能 ensure single-copy consistency
...
26. 但是如果不做协议的话、那么可能会导致每个 replica 受到 request 的顺序不一样（因为广播速度受到距离的影响）

27. 所以可以做 RSM + primary backup replication
...
13. 任何一台机器、都可以是 proposer 又是 acceptor

14. 目标：所有的 acceptor 都接受一个 proposal 的值

15. Quorum = any majority of Acceptors（是一个多数派的概念）
...
18. Paxos has rounds; each round has a unique ID (N)

19. Rounds are asynchronous
...
20. Each round itself broken into phases
...
22. 为什么要多个 acceptors、因为一个可能会 fail

23. 注意这个 prepare 和 accept 的对象也可以是发出这个 proposal 请求的自己

24. 抽屉原理、保证不会有两个 leader 同时看到两个 majority

25. 当 Leader receives a majority <accepted, …>的时候、就可以确定这个 value 是确定的了（只有当这个时间点的时候、才知道有 majority 了，但实际上这个值被确定是在这个时间点之前的某个时刻发生的，但是因为节点间不会一直通信导致的）

26. 有很多数据需要记录在 log 硬盘上、在 ppt 第 50 页上（考虑重启的时候、单个节点从失败中恢复时）

27. 就是一轮里面、可能会有多个里面的值不同、比如 9 个里面 5 个是不同的值、其余 4 个是 null、所以可能最终的值是这个 5 个值里面的任意一个、而不保证是最新的那一个

28. paxos 可能选不出来，没有终止的保证，所以会有出现 view server 对应的 primary 和 backup 的 server 不一致的情况

29. 所以 paxos 的目标和达成了就是让所有的机器都接受同一个值
