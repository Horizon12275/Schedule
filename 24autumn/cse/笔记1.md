...
25. 为了做 operation efficiency，会有 Hash Index (in-memory & on disk) ，在不同条件下结合 log 会有不同的性能特性

26. 为了支持范围查找、会引入 B（+）tree

    - 每个节点都是固定大小的
    - 数据存在叶子节点
    - 支持范围查找（因为有序）
    - 不需要 log-structured，因为 B（+）tree 本身就是有序的
    - 问题：对于 insertion-intensive workload，B（+）tree 性能不够好，因为会有很多的随机 io
    - 对于小的数据、index 的大小可能会和数据一样大

27. SSTables（sorted string table）：把数据按照 key 排序，然后存到磁盘上

    - 优点 1：找 key 很方便（二分查找）。只需要 sparse index （不需要 hash index）（sparse index：只存一部分 key，比如最大最小的 key）
    - 优点 2：支持范围查找
    - 优点 3：merge 的时候很方便（因为是有序的）

28. 怎么保证 SSTable 是有序的呢？

    - 用一个 memtable，把数据先存到 memtable 里，然后再存到 SSTable 里
    - 但是 memtable 会有一个大小限制，如果超过了，就会存到 SSTable 里

29. Reads：先读 memtable，然后读 SSTable，如果 memtable 里没有，就读 SSTable，如果 SSTable 里没有，就读下一个 SSTable

30. 对于单一层的 SSTable，仍然会有缺陷：

    - 找旧的数据会很慢
    - deduplication 会很慢（deduplication：去重）
    - 范围查找还是比较低效

31. 所以引入了 SSTable Hierarchy
...
1. KVS 应该怎么部署？
   ...
   - 方法 1：保存 KVS 在一个 centralized server 上
   - 但是很慢，因为会有很多的网络延迟
...
3. 对于方法 2 中 sync 同步的 naive 方法：
   ...
   - 问题 1：不高效（对于 write）（read 还行）
   - 问题 2：不能容忍网络连接性断连
   - 改进方法 3：写的时候不等待，直接返回，在后台同步
...
9. 最强一致性：strict consistency
   ...
   - 但是实现这个模型会有挑战、因为如果 p0 先发送请求、p1 后发送请求，但是 p1 先到达 server，那么 server 会先处理 p1 的请求，这样就不是 strict consistency 了
...
11. 所以还有了 linearizability
    ...
    - 即可以根据开始-结束时间进行排序
    - 比 sequential 多了一条规则：即如果一个操作在另一个操作发出之前完成，那么后续的操作一定要能看到这个写完的状态
    - 比 sequential 强
...
13. 最简单的实现方式：通过 centralized KVS

    - RPC 有了返回结果即认为完成了

14. 有多个 KVS 的时候，方法一：Primary-backup model
...
21. eventual 认为 conflict write 是比较少见的、所以就先写、然后再做一致化，能提升性能

22. 如果要做一致化、那么这个类似 chat 的就不能做成 KVS 中那些简单的键值的覆盖、而应该做一个 update 的 function
    ...
    - 然后排序 log 的 entry、然后再写到数据库里面（怎么排序：还是根据时间戳）（<time T, node ID>，中、还是先根据 time 比较）
    - 一个问题是：什么时候去 apply 呢？如果同步间隔就的话、一致性差异就大。所以可以先 apply，然后 sync log 之后 rollback

23. 如何选择这个独特的 ID 是比较重要的

    - <time T, node ID>如果用这个的话、还是会依靠于这个 nodes 的物理时钟，不同的 server 的物理时钟也是不同步的
    - 这种不同步的物理时钟、甚至会导致不同的 server 之间会 crash（会 rerun delete before add）

24. causality 因果性 causal ordering

    - 比如在一台机器上面、X 先、Y 后、就是 X->Y
    - 或者在不同的机器上、一个操作是由于另外一个操作发生的 X->Y（X 加、Y 删）

25. 所以一种方法是 X 接受到时间戳之后、更新本地的时间戳到更大的时间戳

26. 解决方法 Lamport Clock

    - 每个 server 维护一个 time T
    - 随着真实时间的增加，T 也会增加（比如 1s 增加一个）
    - 把本地的时间戳和 T 比较，T = max(T, T' + 1)
    - 这就解决了因果性的问题

27. 扩展：实现 partial order： vector clock （缺陷：会比较大）
...
29. For many scenarios, Lamport clock is sufficient

    - Sufficient to order events
    - Save space for storing the clock

30. 我们需要对 log 进行进一步优化（truncate log）

    - 因为每个 write 都是立即 write，所以是 unstable 和 tentative 的 （需要做区分）
    - 为了减少 log 大小
    - 为了减少每次回滚的开销

31. 方法 1：可以做一个 De-centralized 的方法

    - 把某条 log 的 entry 作为一个 checkpoint，如果所有的 server 的时间戳都大于这个时间戳，那么就可以把这个 log entry 设为 stable

32. 但是有一个问题 Problem: If any node is offline, the stable portion of all logs stops growing，所以引入中心化方法

33. 方法 2：centralized approach

    - 引入<CSN, local-TS, SrvID>的 timestamp
    - CSN: commit sequence number
    - 这种方法可能不能保证所有情况下的因果关系、需要做额外的处理来解决这个问题、比如一次拿很多个的操作？

34. 可能会出现一个问题、就是一个 local-ts 大的 server 会先把这个 log 同步到 primary、然后 local-ts 小的会在后面向 primary 做同步，然后最后同步到另一台机器时、会做 reorder updates、导致用户看到的更新顺序不一致

35. An update (W) is stable iff no entries will have a lamport timestamp < W

36. lamport clock 会考

37. If TS(event #1) < TS(event #2), what does it say about event #1 (create on node #1) and event #2 (created on node #2)?
...
38. 这个 consistency anomalies（异常现象）重要嘛？
...
5.  crassh during commit journal？

    - 假设在写磁盘的时候、是 all or nothing 的、那么就不会有问题
...
7.  所以有了 Logging：
    ...
    - 和 journaling 很像，Avoid updating the disk states until we can recovery it after failure
    - 分为 log file 和 log entries
    - 有了 transaction 的概念（begin，commit，abort）
    - 通过 append 日志来记录操作（这里是更新文件）
    - 如何确保一个 log entry 是完整的？（通过一个特殊的标记 checksum（算一遍或者特殊的约定））

8.  每一次操作完的时候、其实不需要直接做 fsync

9.  如果 crash 了，可以再执行一遍 log，然后再执行 commit （redo log）

10. 为什么 commit point 重要？需要确保 all-or-nothing atomicity

11. redo-only 的好处

    - commit 的速度块、只需要 append 操作到日志

12. 但是 redo-only 也有缺点

    - 会有很多的重复磁盘 IO
    - 内存可能会不够用
    - 这个 log file 会不断地变大

13. 为了解决这个问题，引入了 undo-redo logging

    - 记录了所有 update actions
    - 回滚到 old state

14. redo-only 和 undo-redo 的区别

    - redo-only 只 append 了 log entry
    - undo-redo 会 append log records（会出现多个线程的 operation 记录交织的情况）
