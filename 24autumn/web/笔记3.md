...
43. **项目包含的模块**：
    - **Hadoop Common**：提供支持其他 Hadoop 模块的通用工具。
    - **Hadoop Distributed File System (HDFS™)**：一个分布式文件系统，提供高吞吐量的应用数据访问。
    - **Hadoop YARN**：一个作业调度和集群资源管理框架。
    - **Hadoop MapReduce**：一个基于 YARN 的系统，用于并行处理大规模数据集。
    - **Hadoop Ozone**：为 Hadoop 提供的对象存储。
