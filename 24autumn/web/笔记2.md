...
15. 有一个 normal index，还有一个 geospatial index，就可以根据坐标的范围来求最近点之类的内容了

16. 这个貌似有一个就是不加自定义语句的话、就是按照经纬度去计算（貌似只要是一对数值就可以自动进行计算了）、如果加了、就按照二维坐标系里的距离去计算（相当于是二维索引的范围搜索）

17. 注解里的 fields 可以用来做结果的返回的过滤

18. $nearSphere 也可以不止是一个球、比如说正方形之类的

19. 进一步地、这个非关系型数据库就可以解决下面这种很多种枚举的情况、不可能枚举完、而且也不能在关系型数据库里用很多张表去维护（相当于 schema 不一样的时候、做某些操作、比如说搜索的时候、即使有不存在的字段、也不会报错）
